// Canonical shared badge definitions and evaluation logic (WS3-001).
// These define the UI badge system (progress-based, client-evaluated)
// as distinct from server-side achievements (achievement-definitions.ts).
// See §4.1–4.4 of the Insight Points specification.

import type { LucideIcon } from "lucide-react";
import {
  BookCheck, BookMarked, BookOpen, BookOpenCheck, BookText, Brain, Briefcase,
  CalendarCheck, CalendarRange, CircleCheck, Compass, Crosshair, Crown, Diamond,
  Feather, Flag, Flame, FolderOpen, Gem, Globe, GraduationCap, Headphones,
  Landmark, Library, Lightbulb, ListChecks, Medal, Mountain, Notebook,
  NotebookPen, NotebookText, PenLine, Puzzle, Repeat, RotateCcw, Ruler, Scale,
  Search, Shuffle, Sparkles, Star, Swords, Target, TrendingUp, Trophy, Undo2,
...
export type BadgeDefinition = {
  ...
  whyItMatters: string;
  howToEarn: string;
  /** lucide-react icon name (e.g. "Flame"); resolved to a component via BADGE_ICONS. */
  icon: string;
...
// ── Badge icon registry ───────────────────────────────────────────────────────
// Every BadgeDefinition.icon holds a lucide-react icon NAME (kept as a plain
// serializable string so the catalog stays data, not JSX). Render sites resolve
// the name to a component through this map. Standardizing on lucide replaces the
// old OS-emoji glyphs so earned/locked tiles share one icon language with the
// rest of the app (UF-4).
...
/** Fallback icon rendered when a badge.icon name is not found in BADGE_ICONS.
 *  Render sites resolve via `BADGE_ICONS[name] ?? FALLBACK_BADGE_ICON` (a static
 *  member lookup, not a call — keeps react-hooks/static-components happy). */
export const FALLBACK_BADGE_ICON: LucideIcon = Trophy;
