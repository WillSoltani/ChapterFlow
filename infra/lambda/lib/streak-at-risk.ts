...
export async function processStreakAtRisk(
  ...
  // Per-user streak-at-risk nudge: streak Get → dedup Get → in-app notification →
  // best-effort email → dedup marker. Processed with bounded concurrency below
  // instead of a serial per-user await-chain so a large streak cohort can't push
  // the cron past its timeout and drop the tail. Errors are isolated per user
  // ("error") so one bad user can't abort the whole pass.
