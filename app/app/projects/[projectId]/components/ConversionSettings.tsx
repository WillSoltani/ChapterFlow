...
export function ConversionSettings(props: {
  ...
  return (
    <div className="cf-panel rounded-[28px] p-4 sm:rounded-[32px] sm:p-6">
      <div className="flex items-center gap-3">
        <div className="cf-icon-wrap grid h-10 w-10 place-items-center rounded-2xl">
          <SlidersHorizontal className="h-5 w-5 text-[var(--cf-accent)]" />
        </div>
        <div>
          <p className="cf-kicker">Workspace</p>
          <div className="text-base font-semibold text-[var(--cf-text-1)] sm:text-lg">Conversion Settings</div>
        </div>
      </div>

      {/* Format picker — uses shared conversion matrix from backend allowlist */}
      <div className="mt-6">
        <div className="text-sm font-semibold text-[var(--cf-text-1)]">Supported Formats</div>
        {canSelectTargets && props.recommendedFormats.length > 0 ? (
          <div className="mt-2 text-xs text-[var(--cf-text-3)]">
          ...
          {displayFormats.map((opt) => {
            ...
            return (
              <button
                ...
                title={tooltip}
                className={[
                  "cf-btn min-h-10 rounded-2xl px-2.5 py-2 text-xs font-semibold sm:px-3 sm:text-sm",
                  active
                    ? "cf-btn-primary"
                    : selectable
                      ? "cf-btn-secondary"
                      : "border-[var(--cf-border)] bg-[var(--cf-surface-muted)] text-[var(--cf-text-3)]",
                ].join(" ")}
              >
                {opt}
              </button>
        ...
        {!canSelectTargets ? (
          <div className="mt-2 text-[11px] text-[var(--cf-text-soft)]">
            Select a file to choose a target format.
          </div>
        ) : (
          <div className="mt-2 text-[11px] text-[var(--cf-text-soft)]">
        ...
        <>
          <div className="mt-6">
            <div className="flex items-center justify-between">
              <div className="text-sm font-semibold text-[var(--cf-text-1)]">Quality</div>
              <div className="text-sm font-semibold text-[var(--cf-text-1)]">{props.quality}%</div>
            </div>
            <div className="mt-4">
              <input
                type="range"
              ...
              />
              <div className="mt-2 flex items-center justify-between text-xs text-[var(--cf-text-soft)]">
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between">
              <div className="text-sm font-semibold text-[var(--cf-text-1)]">Resize</div>
              <div className="text-sm font-semibold text-[var(--cf-text-1)]">
          ...
          <div className="mt-6">
            <div className="text-sm font-semibold text-[var(--cf-text-1)]">Quick Presets</div>
              ...
              {presets.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      "group w-full rounded-3xl px-4 py-4 text-left transition",
                      active
                        ? "cf-panel-strong border-[var(--cf-accent-border)] ring-1 ring-[var(--cf-accent-soft)]"
                        : "cf-panel-muted hover:border-[var(--cf-border-strong)] hover:shadow-[var(--cf-shadow-lg)]",
                    ].join(" ")}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={[
                          "cf-icon-wrap grid h-10 w-10 place-items-center rounded-2xl transition",
                          active ? "border-[var(--cf-accent-border)] bg-[var(--cf-accent-soft)] text-[var(--cf-accent)]" : "",
                      ...
                      <div className="min-w-0">
                        <div className="text-sm font-semibold text-[var(--cf-text-1)]">{p.title}</div>
                        <div className="text-xs text-[var(--cf-text-3)]">{p.sub}</div>
                      </div>
                    </div>
                  </button>
                );
      ...
      <div className="cf-banner cf-banner-info mt-4 rounded-xl px-4 py-3 text-sm">
        <div className="flex items-start gap-3">
          <div className="mt-0.5 text-[var(--cf-accent)]">ⓘ</div>
