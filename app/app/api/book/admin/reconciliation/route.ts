...
// timeout is hard-set to 30s (infra/lib/chapterflow-frontend-stack.ts ServerFn).
...
// kills the handler at 30s. Raising the shared ServerFn timeout is out (it would
// affect every route); a dedicated reconciliation Lambda needs new infra. Until
// then we keep the work bounded so it finishes inside 30s, and surface
...
// the enforced 30s ServerFn budget. 8 pages × 100 = up to 800 live subs; beyond
 ...
 *
 * Bounded to finish inside the 30s ServerFn timeout (see note above). When the
