 ...
 *
 * GATING — this deliberately runs ONLY the content route's READ authorization
 * (`getUserAccessibleChapter`: requires a started progress row, then enforces
 * `chapterNumber <= unlockedThroughChapterNumber`). It does NOT run
 ...
 * exactly as before.
 *
 * Consequently this hydrates content ONLY for a viewer who is already entitled
 * (has a progress row ⇒ already passed the paywall when they started the book)
 * AND whose target chapter is unlocked. Every other case — not-started /
