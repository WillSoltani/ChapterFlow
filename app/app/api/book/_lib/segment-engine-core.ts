// Pure (AWS-free, server-only-free) segment predicate evaluation, extracted so
// the filter semantics can be unit-tested. `segment-engine.ts` re-exports these
// behind its `import "server-only"` guard; route handlers import from there.
...
function evaluateFilter(user: SegmentUser, filter: SegmentFilter): boolean {
  ...
  switch (filter.field) {
    ...
    case "lastActiveWithinDays": {
      // A user with no recorded activity is "never active". The `gt` operator
      // here means "active WITHIN N days" (t >= cutoff, below), so a never-active
      // user must NOT match it — only an explicit `isEmpty` selects them. The
      // `lt` operator ("inactive for >= N days") is intentionally also a non-match
      // for never-active users so the two are not mutually contradictory: use
      // `isEmpty` to target never-active users specifically.
      if (!user.lastActiveAt) return op === "isEmpty";
