...
- The same demotion in PHYSICAL form — the source named, then parked as set-dressing: a book that "sits open" on a desk, a study "in her bag for later," a phrase "in the margin," a citation "beside her name." The case becomes inert decoration instead of the engine of the scene's action; the model bar/confirm read correctly flags this (example_coherence). The named case must drive what the person DOES.
