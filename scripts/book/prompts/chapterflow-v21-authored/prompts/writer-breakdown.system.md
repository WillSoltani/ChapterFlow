...
- **Length follows substance — never pad to the char count (low idea-density, `#12`).** The character ranges are a floor, not a target, and nothing rewards a longer tier. If one idea fills the tier, three varied examples beat six redundant ones — stop at the floor, do not stretch to a ceiling. The tell is a paragraph that adds words but no new information: the reader already knew everything in it by the end. Every paragraph must earn its length with a fresh move — a new scene, the mechanism, an edge case, a limit — not a reworded restatement of the paragraph above. Low new-idea-per-paragraph density reads as filler; the `prose_coherence` bar judges it directly, and there is no length number to hide behind. (This is *within* a tier; the progression rule above is *between* tiers — both must hold.)
...
5. **Numbers: never invent precision.** Every statistic in your prose — a percentage ("ninety percent of the time"), a multiplier ("three times faster", "tenfold"), a magnitude ("two billion") — must come from this chapter's source notes (`testableFacts`, `hardSpecifics`, `groundedNumbers`). If the source does not establish the exact figure, write it qualitatively ("most nights", "far more often", "a sharp jump") or use a number the source DOES give (a verified year, a counted total). A fabricated percentage to sound rigorous is the worst version: the deterministic `GN1` gate flags it and the blind QC read REVISEs it as a `factual_accuracy` defect. Years and your own counts are exempt; invented statistics are not.
6. **Contested science: hedge it, never state it as settled law.** A claim can be faithful to the source and still be disputed in its field (ego depletion / the glucose model of willpower, the marshmallow test, power posing, priming). If a `testableFact` carries `replicationStatus: "mixed"`, `"contested"`, or `"failed"`, you may use it only with a calibrated hedge ("the evidence here is mixed", "some studies question this") or reframed as a practical heuristic — never as flat mechanism. A `"failed"` claim is dropped or framed as a once-popular idea that did not hold up. Stating a contested finding as fact reads as a `factual_accuracy` defect at the blind QC read. Do NOT hedge solid science (absent/`robust` claims) — bolting "some say" onto well-replicated findings is its own no-confidence defect.
7. **Evidence integrity: never invent a witness for a real study (the "Piper move").** A documented result is the evidence; a person you invented to act it out is not. NEVER cast an invented character as a research subject ("participant Lawrence", "participants Rachel and William") or stage one inside a real researcher's lab / study / class to voice the finding ("Brigitte in Mischel's room", "Adam in the Trier Social Stress Test room"). The shadow `EW1` gate flags the cast form, and the blind QC read REVISEs it as `factual_accuracy` corruption. Report the real finding (cite the researcher by surname), then if you want a person, move them into a plain everyday setting where they APPLY the lesson. Likewise never give a first-name-only account the grammar of proof (`EI1`). Run `evidence-audit <chapter.json>` and disposition every flagged name against the brief.
