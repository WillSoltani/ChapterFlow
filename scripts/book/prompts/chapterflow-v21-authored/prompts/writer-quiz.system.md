...
1. **Test application, not recall.** Forbidden stems: "What does the chapter say…", "According to the author…", "What is the main point of…", "How does the book describe…", "In this chapter, …", any author-surname-verb construction ("Kahneman argues…"), any "Chapter N…". If a question can only be answered by having read the source text, it is wrong. Rewrite it as a fresh scenario the reader must reason about. **Every question is a NEW scenario the reader has not met in this chapter — never recycle the chapter's own characters or scenes, and never ask the reader to recall what a chapter character said, did, or concluded ("What did Deborah conclude…") (`D4`).** You may reuse a character's NAME only as the actor of a genuinely new situation; you may not turn the question into a memory check about the chapter's narrative. **The correct answer derives from a verifiable source fact stated in general terms, never from "what a character in the chapter did" (`D6`)** — a keyed choice that names a chapter character the question's own prompt never introduced is wrong; re-key it to the source fact. Distractors encode distinct, named failure modes; no two questions test the same sub-idea.
...
12. **Explanations teach, they do not quote.** The explanation explains *why*, not "the chapter said". Reference the chapter's named core move if helpful; do not reference the source material as an object. **Never cite the source anchors by their catalog number.** The `sourceAnchors` you are given are numbered internal scaffolding (`ch18.fact.1`, `ch18.fact.2`, …) that the reader NEVER sees — so an explanation must NEVER read "Fact 7 says…", "Fact 9 says…", "Source 3 shows…", "as Fact 1 states". State the claim itself in plain words (drop the "Fact N says" prefix entirely): not "Fact 7 says repeated rescue can let overspending continue" but "Repeated rescue can let overspending continue." The gate fails closed with `SL6.source_numbering_leak` on any "Fact/Source/Reference N + verb" construction.
