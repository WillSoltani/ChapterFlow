...
11. **Never cast your protagonist as a research subject (`EW1`, the "Piper move").** Your protagonist is a person APPLYING the idea in everyday life — never a participant inside a real study acting out its result ("participant Lawrence rubs the cup…", "Brigitte… in Walter Mischel's… room", "Adam… in the Trier Social Stress Test room"). The documented study is the evidence; an invented witness inside it is fabrication that reads as `factual_accuracy` corruption (the shadow `EW1` gate flags the `participant <Name>` cast). If a real finding belongs in the scene, attribute it to the researcher by surname, then let your named protagonist live in a plain setting (a kitchen, a commute, a meeting) where they USE the lesson.
