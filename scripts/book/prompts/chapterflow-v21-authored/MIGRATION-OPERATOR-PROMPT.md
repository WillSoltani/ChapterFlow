...
- **Claude session, free** (default): no env override — uses Anthropic Code CLI on the Max subscription.
- **Anthropic API, paid**: `export CHAPTERFLOW_PROVIDER=anthropic-api` (uses `ANTHROPIC_API_KEY`)
- **OpenAI API, paid**: `export CHAPTERFLOW_PROVIDER=openai-api CHAPTERFLOW_WRITER_MODEL=gpt-5.5`
...
Should print `{"ok": true, ...}`. If not, stop and tell the user.
...
## Step 8 — Wire into the local bookPackages registry (optional but recommended)

Skip this if you're confident the production catalog will serve the book to all users. Add it if you want the book to render in localhost dev too:
