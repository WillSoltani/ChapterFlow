...
## v18 Assembly Hygiene Checks

Mechanical hard-fail if any chapter contains:
- non-canonical keys inside `contentVariants`
- both `author` and `authors` in a chapter or review wrapper unless the manifest explicitly requires both
- sibling `moreDetails`
- `structuredRecap`
- `takeaways` alongside `keyTakeaways`
- scenario as plain string
- quiz with fewer than 10 questions
- any worker-instruction leakage phrase such as:
...
## Prose hard-fail additions

Flag for repair if any are true:
- core thesis explicitly restated more than 4 times across the assembled chapter
- medium and hard teach the same concept set with denser wording only
- two or more `point` fields in the same depth could be merged without loss
- examples feel like principle delivery vehicles instead of scenes with concrete human tension
- the chapter contains obvious leftover assembly seams or duplicate sections
