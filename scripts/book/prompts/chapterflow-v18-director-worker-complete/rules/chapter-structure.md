...
## v18 Canonical Field Hygiene

Only these top-level chapter keys are allowed:
- `chapterId`
- `number`
- `title`
- `readingTimeMinutes`
...
Within `contentVariants`:
- Easy allows ONLY: `chapterBreakdown`, `keyTakeaways`, `oneMinuteRecap`
- Medium allows ONLY: `chapterBreakdown`, `keyTakeaways`, `activationPrompt`, `selfCheckPrompt`, `oneMinuteRecap`
- Hard allows ONLY: `chapterBreakdown`, `keyTakeaways`, `activationPrompt`, `selfCheckPrompts`, `predictionPrompt`, `oneMinuteRecap`
...
- `summary`
- `whatChanges`
- duplicate `author`/`authors` metadata inside chapter payloads
- any extra wrapper fields inside `contentVariants`

## Separation Of Work

The structure agent may write:
- `contentVariants`
- `implementationPlan`
- `reviewCards`
- `keyTakeawayCard`
...
Those are written by dedicated workers and assembled later.

## Repetition Guard

- The first sentence of each `moreDetails` must extend the parent point rather than restate it.
- If two `point` fields teach the same claim with swapped adjectives, merge or rewrite.
- Medium and hard must differ by concept, not pressure alone.
