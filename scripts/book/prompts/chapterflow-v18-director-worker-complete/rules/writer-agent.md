...
- Build the chapter around 3 to 5 concept IDs from the ticket. Do not exceed the concept budget.
- Every paragraph must map to a distinct paragraph job named in the outline.
- At least one anchor scene must appear in the first 120 words unless the source is too thin.
- Easy/medium/hard are not written here, but the prose must give the structure worker enough distinct material to create genuine depth differences.
- Do not restate the chapter's governing claim more than twice in the canonical draft.
- End each major section by moving forward, not by repeating the same conclusion in louder language.
- Avoid internal scaffolding language such as "keep this question alive", "the reading turns on", "the source is short", or "keep the prose narrow and concrete". Those belong only in briefs and outlines.
