...
- Never copy brief instructions or calibration notes into reader-facing fields.
- Never paste raw source lines into teaching fields unless explicitly allowed as a quote in the brief.
- `quiz` must not be null in flagship mode.
- `examples[].scenario` must be tone objects when scenarioTonePolicy = required.
