...
### QC-visible hard specifics — mandatory calibration

The first QC round checks factual accuracy against the source sidecar and the source-verify record. It cannot reward facts that are true but hidden. For every chapter:

- Put all likely prose facts into explicit fields: `testableFacts[]` for teachable propositions and `namedExamples[].hardSpecifics[]` for concrete tokens tied to cases. Do not leave important quantities only in `summary` or `paraphraseNotes`.
- Include spelled-out quantities and durations as hard specifics too: `four acute care hospitals`, `over four years`, `ten minutes`, `two departments`. QC treats these as factual numbers even when no digit appears.
- Every `hardSpecifics[]` item must be independently verifiable in `.chapterflow/source-verify-<bookId>.md`; do not add plausible details merely to make the sidecar dense.
- When the same institution/person/case appears in more than one chapter, note which metadata should not be repeated. Repeated location/date/scale stamping creates first-QC `location_stamping` repairs even when the facts are true.
- Use `forbiddenLeakage` to prevent cross-unit imagery bleed: name the adjacent case, venue, or stakes that must not appear in this chapter.

Research quality is measured by how little the writer must invent and how easily QC can verify the finished prose.

Before any writer fanout, run:
```bash
npx tsx src/cli.ts source-v2-gate <bookId> --prewrite
```
Treat every blocker here as a research defect. In particular, `SV2.realness_unsupported_entity` means
a real-world named example lists hard specifics that are generic or not supported by its own summary/
paraphrase notes. Repair by adding verified concrete details and making those details visible in the
sidecar prose, or replace the broad entity with a concrete verified case. Do not let the writer solve
this by inventing specificity.
