...
export async function generateChapter(
  ...
  const policy = options.runPolicy ?? defaultRunPolicy("standard");
  const CANDIDATES_PER_SLOT = options.candidatesPerSlot ?? policy.examples.maxCandidates;
  const VOICE_PASS_MAX = options.voicePassMaxIterations ?? policy.prose.maxVoicePasses;
  ...
  for (let iter = 2; iter <= VOICE_PASS_MAX; iter++) {
    ...
    const proseRisk = scoreProseIssues(issues);
    if (proseRisk.severe.length === 0 && proseRisk.score < policy.prose.lineEditorRiskThreshold) {
      log(`voice pass: ${issues.length} low-risk finding(s), leaving to line-editor/QC instead of full rewrite`);
      break;
    }
    log(`voice pass iter ${iter}: ${issues.length} findings (risk=${proseRisk.score}), re-running`);
    try {
  ...
  // Final line-editor pass. v22 makes this risk-gated by policy: clean chapters
  // keep their voice-passed prose; risky chapters still get the surgical polish.
  const lineEditorIssues = runProseChecksOnBreakdown(breakdown);
  const lineEditorRisk = scoreProseIssues(lineEditorIssues);
  const shouldRunLineEditor =
    policy.prose.lineEditor === "always" ||
    (policy.prose.lineEditor === "risk" && lineEditorRisk.score >= policy.prose.lineEditorRiskThreshold);

  if (!shouldRunLineEditor) {
    log(`line editor: skipped by ${policy.name} policy (prose risk=${lineEditorRisk.score}, findings=${lineEditorIssues.length})`);
    recordGenerationStage(generationManifest, {
      stage: "line-editor",
      input: { brief, plan, draftHash: generationInputHash(breakdown), policy: policy.name, proseRisk: lineEditorRisk },
      output: { skipped: true, reason: "below-risk-threshold" },
    });
  } else {
    log(`line editor: surgical polish pass (risk=${lineEditorRisk.score}, findings=${lineEditorIssues.length})…`);
    try {
      const beforeLineEditor = breakdown;
      breakdown = await agents.runLineEditor({ brief, plan, draft: breakdown });
      recordGenerationStage(generationManifest, {
        stage: "line-editor",
        input: { brief, plan, draft: beforeLineEditor, policy: policy.name, proseRisk: lineEditorRisk },
        output: breakdown,
      });
  ...
  // Examples. v21 always over-generated 3 candidates per slot. v22 keeps
  // quality through deterministic scoring + selective escalation: strong first
  // candidates are accepted, weak slots buy more candidates, ties/risky slots
  // still go to the model curator.
  log(`examples: ${policy.examples.strategy} generation, ${plan.exampleCount} slot(s), ${policy.examples.minCandidates}-${CANDIDATES_PER_SLOT} candidate(s)/slot…`);
  ...
  for (let i = 0; i < plan.exampleSpecs.length; i++) {
    ...
    const t0 = Date.now();
    const slotAnchors = selectAnchorsForClaim(sourceEvidence, ["example"], spec.sourceAnchorIds, 4);

    const runCandidates = async (count: number, namesList: string[]) => {
      const promises = Array.from({ length: Math.max(0, count) }, () =>
    ...
    const initialCount = policy.examples.strategy === "fixed"
      ? CANDIDATES_PER_SLOT
      : Math.min(CANDIDATES_PER_SLOT, Math.max(1, policy.examples.minCandidates));
    let { candidates, errors } = await runCandidates(initialCount, usedNames);
    ...
    if (candidates.length === 0) {
      ...
      if (noisyFraction >= 0.5) {
        log(`  example[${i}] initial candidates failed on pronoun/noise names, filtering ledger and retrying…`);
        const cleaned = usedNames.filter((n) => !/^(You|Your|We|Us|Our|My|I|Me|Him|Them|Who|What|Why|How|He|She|They|It|This|That|These|Those|Here|There|Not|Nobody|Anybody|Somebody|Everyone|Anyone|Someone|None|Yes|No|Maybe|Once|Only|Even|Also|Still|Again|Just|When|Where|While|Before|After|During|Until|Since|And|But|Or|So|If|Because|Then|Now|Today|Tomorrow|The|A|An|First|Second|Third|Fourth|Fifth|Last|Next)$/.test(n));
        const retry = await runCandidates(initialCount, cleaned);
        const event = recordGenerationDegradation(generationManifest, {
          stage: "writer-example",
          inputHashes: generationInputHashes({ spec, specIndex: i, usedNames, initialErrors: errors, policy: policy.name }),
        ...
        log(`  example[${i}] initial candidates failed: ${errors.slice(0, 3).join(" | ")}`);
        log(`  example[${i}] retry with relaxed name constraints (within-chapter only, no cross-book forbidden)…`);
        const retry = await runCandidates(initialCount, withinChapterNames);
        const event = recordGenerationDegradation(generationManifest, {
          stage: "writer-example",
          inputHashes: generationInputHashes({ spec, specIndex: i, usedNames, initialErrors: errors, policy: policy.name }),
    ...
    if (policy.examples.strategy === "adaptive" && candidates.length > 0 && candidates.length < CANDIDATES_PER_SLOT) {
      const scoredInitial = candidates.map((candidate) => ({
        candidate,
        score: scoreExampleCandidate(candidate, {
          spec,
          specIndex: i,
          usedNames,
          priorExamples: examples,
          sourceAnchors: slotAnchors,
        }, { acceptScore: policy.examples.acceptScore, retryScore: policy.examples.retryScore }),
      }));
      const bestInitial = scoredInitial.slice().sort((a, b) => b.score.score - a.score.score)[0];
      const deterministicInitial = chooseDeterministicExampleWinner(scoredInitial, policy.examples.deterministicMargin);
      const canAcceptInitial = !!bestInitial && bestInitial.score.accept && (candidates.length === 1 || deterministicInitial.winner !== null);
      if (!canAcceptInitial) {
        const remaining = CANDIDATES_PER_SLOT - candidates.length;
        log(`  example[${i}] first pass score=${bestInitial?.score.score ?? 0}; generating ${remaining} more candidate(s)…`);
        const extra = await runCandidates(remaining, usedNames);
        candidates = [...candidates, ...extra.candidates];
        errors = [...errors, ...extra.errors];
      }
    }

    if (candidates.length === 0) {
      log(`  example[${i}] all candidates failed: ${errors.slice(0, 3).join(" | ")}`);
    ...
    const deterministic = chooseDeterministicExampleWinner(scored, policy.examples.deterministicMargin);
    const best = scored.slice().sort((a, b) => b.score.score - a.score.score)[0];
    if (!best) throw new Error(`example[${i}] scorer had no candidates after non-empty candidate validation`);

    let winner: ExampleOutput;
    let selectionMode = "deterministic";
    const curatorRequired =
      policy.examples.curator === "always" ||
      (policy.examples.curator === "risk" && (best.score.needsCurator || deterministic.winner === null)) ||
      (policy.examples.curator === "tie" && deterministic.winner === null);

    if (candidates.length === 1 || !curatorRequired) {
      winner = deterministic.winner ?? best.candidate;
    } else {
      const curated = await agents.runExampleCurator({ brief, plan, spec, candidates });
      winner = candidates[curated.winnerIndex] ?? best.candidate;
      selectionMode = "curator";
    }

    examples.push(winner);
    recordGenerationStage(generationManifest, {
      stage: "writer-example",
      input: { spec, specIndex: i, policy: policy.name, candidateScores: scored.map((s) => ({ score: s.score.score, findings: s.score.findings })) },
      output: winner,
    ...
    log(`  example[${i}] [${spec.format}] ${selectionMode} score=${best.score.score} candidates=${candidates.length} "${winner.title}" (${((Date.now() - t0) / 1000).toFixed(1)}s)`);
  }
  ...
  // Memorable lines. v22 can select these deterministically; model escalation
  // remains available for premium/publish or when the deterministic selector is weak.
  log(`memorable lines: policy=${policy.support.memorableLines}…`);
  let memorableLines = selectMemorableLinesDeterministic(draftAssembled);
  const deterministicMemorableLines = memorableLines;
  const shouldRunMemorableLineModel =
    policy.support.memorableLines === "model" ||
    (policy.support.memorableLines === "model-on-weak" && memorableLines.length < 3);

  if (!shouldRunMemorableLineModel) {
    recordGenerationStage(generationManifest, {
      stage: "memorable-lines",
      input: { chapterHash: generationInputHash(draftAssembled), policy: policy.name },
      output: { memorableLines, deterministic: true },
    });
    log(`memorable lines: ${memorableLines.length} selected deterministically`);
  } else {
    try {
      ...
      recordGenerationStage(generationManifest, {
        stage: "memorable-lines",
        input: { chapterHash: generationInputHash(draftAssembled), policy: policy.name, deterministicCandidateCount: deterministicMemorableLines.length },
        output: ml,
      });
      log(`memorable lines: ${memorableLines.length} marked by model`);
    } catch (err) {
      const event = recordGenerationDegradation(generationManifest, {
        stage: "memorable-lines",
        inputHashes: generationInputHashes({ draftAssembled, deterministicMemorableLines }),
        error: err,
        attemptCount: 1,
        fallbackUsed: {
          kind: "deterministic-quotable-lines",
          policy: "availability",
          reason: "Memorable-lines model stage failed; used deterministic selector rather than omitting optional highlights.",
        },
        fallbackOutput: deterministicMemorableLines,
        severity: "advisory",
        requiredDisposition: "visible_advisory",
      });
      memorableLines = deterministicMemorableLines;
      log(`memorable lines model failed: ${(err as Error).message}; used deterministic fallback; recorded ${event.eventId}`);
    }
  }
  ...
  if (!gate.passed) {
    // Save the failed draft to a quarantine path so it can be inspected, then
    // write a self-contained repair prompt for the next repair agent. A blocked
    // chapter must never leave only an error string; autonomy needs a durable
    // instruction packet with the exact units, evidence, and validation commands.
    const quarantineDir = resolve(stateRoot, "chapters", "_blocked");
    mkdirSync(quarantineDir, { recursive: true });
    const quarantinePath = resolve(quarantineDir, `${chapter.chapterId}.blocked.${Date.now()}.json`);
    writeFileSync(
      quarantinePath,
    ...
    const repair = writeSelfHealingRepairPrompt({
      ...
      stage: "ship-gate",
      severity: "blocker",
      chapter: { chapterId: chapter.chapterId, chapterNumber: chapter.chapterNumber, chapterTitle: chapter.chapterTitle },
      summary: `Ship gate blocked ${chapter.chapterId}: ${gate.blockers.length} blocker(s), ${gate.majors.length} major(s).`,
      findings: [...gate.blockers, ...gate.majors].map((f) => ({
        id: f.catalogId,
        severity: f.severity,
        unit: f.unit,
        message: f.message,
        evidence: f.evidence,
        expectedFix: "Edit only the named chapter unit so the ship gate finding clears; preserve unrelated reader-facing fields and source anchors.",
      })),
      artifacts: [quarantinePath],
      validationCommands: [
        `npx tsx src/cli.ts author-check state/chapters/${chapter.chapterId}.v21-native.chapter.json`,
        `npx tsx src/cli.ts gate-chapter state/chapters/${chapter.chapterId}.v21-native.chapter.json`,
        `npx tsx src/cli.ts book-gate ${book.bookId}`,
      ],
      stateRoot,
    });
    throw errorWithRepairPrompt(`Ship gate BLOCKED ${chapter.chapterId}: ${gate.blockers.length} blocker(s). See quarantine.`, repair);
  }
  ...
  if (boundaryProblems.length > 0) {
    const quarantineDir = resolve(stateRoot, "chapters", "_blocked");
    mkdirSync(quarantineDir, { recursive: true });
    const quarantinePath = resolve(quarantineDir, `${chapter.chapterId}.boundary.${Date.now()}.json`);
    writeFileAtomic(
      quarantinePath,
    ...
    const repair = writeSelfHealingRepairPrompt({
      ...
      severity: "blocker",
      chapter: { chapterId: chapter.chapterId, chapterNumber: chapter.chapterNumber, chapterTitle: chapter.chapterTitle },
      summary: `Boundary validation blocked ${chapter.chapterId}: ${boundaryProblems.length} problem(s).`,
      findings: boundaryProblems.map((problem) => ({
        id: "boundary-validation",
        severity: "blocker",
        message: problem,
        expectedFix: "Repair identity/source/plan alignment without changing unrelated content; rerun the boundary validation commands.",
      })),
      artifacts: [quarantinePath],
      validationCommands: [
        `npx tsx src/cli.ts author-check state/chapters/${chapter.chapterId}.v21-native.chapter.json`,
    ...
    throw errorWithRepairPrompt(`Boundary validation BLOCKED ${chapter.chapterId}: ${boundaryProblems.slice(0, 5).join("; ")}. See quarantine.`, repair);
  }
