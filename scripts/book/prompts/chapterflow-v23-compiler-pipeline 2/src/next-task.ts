...
export function formatNextTask(task: NextTask): string {
  ...
  switch (task.kind) {
    ...
    case "research-chapter":
      ...
      lines.push("");
      lines.push(`Read playbook: ${task.playbook}`);
      lines.push(`Read schema:   prompts/researcher-chapter.system.md`);
      lines.push("");
    ...
    case "write-chapter":
      ...
      lines.push(`  npx tsx src/cli.ts gate-chapter ${task.outputPath}`);
      lines.push("");
    ...
    case "derive-artifacts":
      ...
      lines.push(`  npx tsx src/cli.ts derive-artifacts ${task.bookId}`);
      lines.push("");
    ...
    case "finalize":
      ...
      lines.push(`  npx tsx src/cli.ts generate-book ${task.bookId} \\`);
