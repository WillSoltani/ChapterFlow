...
function compileFixture(ch: ChapterSpec) {
  ...
  // The section gate now runs on every assembleSections() call (this fixture asserts on it
  // directly below), so text that would otherwise be byte-identical across the two fixture
  // chapters must diverge enough to clear the cross-chapter duplicate-content checks
  // (SEC81/SEC82/SEC94). Only chapter-2 wording differs from chapter-1; everything else in
  // this fixture is unchanged.
  const isCh1 = ch.chapterNumber === 1;
  ...
  const learning: LearningPackV1 = {
    ...
    cards: {
      cards: blueprint.sections.cards.map((c) => isCh1 ? {
        cardId: c.cardId,
        sourceAnchorIds: [aid],
        front: `What credit-card signal should you inspect before a snapshot ${c.cardId}?`,
        back: "Inspect the balance or utilization that would be visible to the reporting system, because that is the account information lenders can later read.",
        difficulty: c.difficulty,
      } : {
        cardId: c.cardId,
        sourceAnchorIds: [aid],
        front: `Which cash-handling habit protects tomorrow's plan, card ${c.cardId}?`,
        back: "Count what remains and separate rent money from grocery money by hand, since paper cash carries no automatic ledger entry.",
        difficulty: c.difficulty,
      }),
    },
  };
  const action: ActionPackV1 = {
    ...
    chapterId: blueprint.chapterId,
    tryThisNow: isCh1
      ? "Open one card account, find the balance that would be visible today, and choose one payment or alert that makes the next snapshot less noisy."
      : "Count the cash in your wallet tonight, separate it into named envelopes, and decide which envelope pays first this week.",
    tryThisNowSourceAnchorIds: [aid],
    implementationPlan: {
