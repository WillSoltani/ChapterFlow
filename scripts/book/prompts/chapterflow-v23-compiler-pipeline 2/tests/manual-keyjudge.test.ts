...
test("manual keyjudge: subset resolution does not clobber unreviewed sibling chapters", () => {
  const OLD = "r-key-subset-old";
  const NEW = "r-key-subset-new";
  const cleanup2 = () => {
    rmSync(resolve(REPO_ROOT, ".chapterflow/runs", BOOK), { recursive: true, force: true });
    for (const r of [OLD, NEW]) {
      rmSync(keyPackDir(BOOK, r), { recursive: true, force: true });
  ...
  const answersFor = (chapter: ReturnType<typeof makeChapter>, round: string): string => {
    ...
    const p = resolve(TMP_DIR, `${BOOK}.answers.json`);
    writeFileSync(p, JSON.stringify({ chapters: [{ chapterNumber: chapter.number, packHash, answers }] }, null, 2), "utf8");
    return p;
  };
  try {
    cleanup2();
    ...
    writeFixtureBook(STATE_CHAPTERS, [ch1, ch2]);
    const sourceDir = resolve(REPO_ROOT, ".chapterflow/runs", BOOK, RUN, "sidecars/source");
    mkdirSync(sourceDir, { recursive: true });
    writeResearchRunManifestFixture({
      runDir: resolve(REPO_ROOT, ".chapterflow/runs", BOOK, RUN),
    ...
    for (const n of [1, 2]) writeFileSync(resolve(sourceDir, `ch${String(n).padStart(2, "0")}.source.json`), JSON.stringify(sourceSidecar(n), null, 2), "utf8");

    const old = openQcRound(BOOK, OLD);
    writeKeyPacks(BOOK, OLD, [ch1]);
    const oldAnswers = answersFor(ch1, OLD);
    validateAndWriteKeyDerivation(BOOK, OLD, "keyA", old.tokens.keyA, oldAnswers);
    validateAndWriteKeyDerivation(BOOK, OLD, "keyB", old.tokens.keyB, oldAnswers);
    assert.equal(resolveManualKeyJudges(BOOK, OLD, [ch1]).records[0].status, "PASS");

    const before = JSON.parse(readFileSync(manualKeyJudgePath(BOOK, 1), "utf8"));
    const fresh = openQcRound(BOOK, NEW);
    writeKeyPacks(BOOK, NEW, [ch2]);
    const newAnswers = answersFor(ch2, NEW);
    validateAndWriteKeyDerivation(BOOK, NEW, "keyA", fresh.tokens.keyA, newAnswers);
    validateAndWriteKeyDerivation(BOOK, NEW, "keyB", fresh.tokens.keyB, newAnswers);
    const resolved = resolveManualKeyJudges(BOOK, NEW, [ch2]);
    assert.deepEqual(resolved.records.map((r) => r.chapterNumber), [2]);
    assert.equal(resolved.records[0].status, "PASS");
    const after = JSON.parse(readFileSync(manualKeyJudgePath(BOOK, 1), "utf8"));
    assert.equal(after.status, "PASS");
    assert.equal(after.roundId, OLD);
    assert.equal(after.resolvedAt, before.resolvedAt, "subset resolve must not rewrite an unreviewed sibling record");
  } finally {
    cleanup2();
  }
});
