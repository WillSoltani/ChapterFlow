...
test("dirtyVsHead: returns registry files that DIFFER FROM HEAD (modified/untracked/renamed), not just what one register-web call changed", () => {
  ...
  const booksJson = resolve(REPO_ROOT, "books.json");
...
test("dirtySourceOutsidePlan: flags dirty source outside the plan; ignores state, the plan itself, and non-source", () => {
  const inPlan = resolve(REPO_ROOT, "src/qc/publishAfterQc.ts");
  const status = [
    " M src/qc/sweep.ts", // dirty source outside plan → flagged
    " M state/gate-attempts.json", // generated state → ignored
    " M src/qc/publishAfterQc.ts", // in the plan → ignored
