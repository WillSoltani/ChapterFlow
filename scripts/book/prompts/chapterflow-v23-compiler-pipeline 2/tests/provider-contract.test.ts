...
function marker(line) {
  if (process.env.CHAPTERFLOW_FAKE_SDK_MARKER) {
    fs.mkdirSync(path.dirname(process.env.CHAPTERFLOW_FAKE_SDK_MARKER), { recursive: true });
    fs.appendFileSync(process.env.CHAPTERFLOW_FAKE_SDK_MARKER, line + "\\n");
  }
}
function nextCall() {
