...
### First-QC readiness audit — mandatory before you submit

The goal is not merely to pass deterministic `gate-chapter`; the goal is for the first independent QC round to pass without repair. Perform this audit after the chapter gate is clean and before reporting done.

1. **Factual accuracy as QC sees it.** QC reviewers see the chapter, sidecar facts, grounded numbers, and source-verify evidence. They do not trust a hard detail just because it sounds true. Every count, duration, date, institutional size, location-specific claim, named framework member, and quoted/paraphrased factual detail must be visible in `testableFacts[]`, `namedExamples[].hardSpecifics[]`, or `.chapterflow/source-verify-<bookId>.md`. If a number is written as a word (`four`, `twelve`, `over four years`), treat it like a number. If it is not visible to QC, remove it or soften it.
2. **Behavioral naturalness.** `tryThisNow`, `whatToDo`, `weeklyPractice`, and the 24-hour challenge must be useful actions a normal reader could take. Do not prescribe symbolic staging, ritualized movement, theatrical object placement, or vague rehearsal unless the source clearly teaches it and the action changes behavior.
3. **Source-local coherence.** Read each example sentence-by-sentence. Every image, stake, noun, and consequence must belong to that example's source unit. Cross-unit bleed is a QC blocker: a clinic discharge scene cannot suddenly mention a ride leaving or rent overdraft; a school example cannot inherit factory-dashboard stakes unless that is the source case.
4. **No repeated source stamping.** If multiple chapters use the same famous case or institution, do not repeat the same location/year/scale facts unless they do new explanatory work. One anchor is enough. Repeated `Moline, Illinois`, `One John Deere Place`, `San Diego`, founding-year, or employee-count style stamps are `location_stamping` findings.
5. **Quiz/card/plan realism.** The blind key judge can derive each quiz key from source facts alone; distractors are same-kind; card backs answer the fronts; implementation steps are concrete, non-generic, and tied to this chapter's mechanism.

If this audit finds an issue, repair the chapter before handing it to QC. Never weaken gates, invent source facts, edit QC attestations, or rely on the first QC round as the repair phase.
...
5. **Keep the cast small and the quiz cast consistent (`C24` / `C25`).** Two failures the gates now catch:
   - **≤1 protagonist per example, ≤6 named people per chapter (`C24`).** Each example has exactly one named lead (a named *foil* is fine, but don't crowd a scene with three named coaches). Across the whole chapter, keep the total distinct named people at six or fewer — demote minor walk-ons to unnamed roles ("a colleague", "the new hire"). A crowd of interchangeable names blurs which person carries each lesson (one regen chapter shipped nine faceless coaches; the gate caps the cast at six). Six is a CAP, not a target — prefer a small cast of two to four leads you actually develop over a fresh face in every scene. A character defined only by a sensory tic (the squeaking marker, the jammed drawer, the toe-hooked sneaker) is a slot, not a person; give each named lead a real stake and a decision, or demote them to an unnamed role. Interchangeable single-use names with a quirk stapled on read as machine-generated, not authored.
...
- **A real action, not a mantra.** The move must be something the reader DOES and could prove they did — move an object, write a line, send the message, schedule the block, ask the question. NOT a verbal affirmation recited to oneself ("say aloud, 'I am enough'", "repeat the mantra", "tell yourself it will pass"). A say-aloud recitation is performative theater, not a doable action, and grades as a `behavioral_naturalness` miss — the same rule binds the `twentyFourHourChallenge` and every if-then action.
...
13. **Teach a spine number, don't assert it.** When one figure is the chapter's backbone — a doubling, a compounding rate, a multiplier — walk the reader through HOW it gets there in `deepRead`/`fullRead` (the first step, the mechanism, the result), never drop the conclusion as a bare assertion ("the penny becomes $10 million"). And state its TRUE magnitude: a 1%/day gain compounds to roughly 37x across a year, so never round it down to "more than triple" — an accurate big number teaches, a lazy small comparison misleads. A beginner needs the hand-hold exactly where the math is the point.
...
3. `ifThenPlans` are 3-5 items, and ALL of them apply the SAME named move from your `implementationPlan.title` to a DIFFERENT trigger — one tool used several ways, not a list of unconnected tips. Each = a concrete trigger ("If your inbox has more than 20 unread items by 10 AM…") + a concrete action that PERFORMS that named move ("…then run the load-draw: write the three places the work is waiting and close the first one."). An if-then whose action ignores the chapter's named tool is a `plan_actionability` REVISE — the reader is left with disconnected advice instead of one practiced loop. The opposite failure is just as bad: applying the move means ADAPTING it to each trigger in freshly-written words, never pasting the same action clause under every trigger. Four plans whose action is one sentence with only the trailing words swapped ("write the Boring Repeat line and …" under every trigger) is a *collapsed* plan — the move named four times, not practiced four ways. Write each action fresh from its own trigger.
4. `twentyFourHourChallenge` is one specific 24-hour commitment with a verifiable outcome — a real doable action, never a say-aloud affirmation (same rule as `tryThisNow`, Step 3).
