...
const COMPLAINTS: Record<number, string[]> = {
  9: [
    "quiz Q1: the prompt 'Which earlier choice caused it?' offers three SIBLING cause-framings — the key (no named person/number/return before the aim moved) and the distractors (store teams outside the return; growth story treated as proof) are all defensible causes from the prose, so two independent blind key-derivators could not agree. Rephrase the PROMPT so the stored key is the one answer the prose directly supports (e.g. ask what was MISSING at the moment the target was set — the key names exactly that absence), and sharpen each distractor so a careful reader can point to a specific sentence that rules it out while it stays tempting to a skimmer. Keep the same choices' substance, the same correctIndex, and do not change any other question.",
  ],
};
