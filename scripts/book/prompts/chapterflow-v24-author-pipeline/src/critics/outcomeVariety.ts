...
const C28_FIX =
  "Make at least one scene per chapter show a failed first attempt, a relapse, a cost, or a constrained result. Uniform instant success reads as survivorship gloss and makes the reader feel like the failure when the move doesn't work the first time.";
...
export function checkOutcomeVariety(chapter: ChapterV21): CriticFinding[] {
  ...
  return [
    finding(
      "C28.uniform_success",
      "major",
      `examples: all ${hit.exampleCount} example scenes resolve in clean instant success — none shows a failed first attempt, relapse, setback, cost, or constrained result — e.g. "${truncate(hit.evidence, 60)}" (no friction-bearing format [mistake_recovery/postmortem] and no cost/failure cue in any scenario). ${C28_FIX}`,
      hit.evidence,
    ),
  ];
}
