...
// the research run recorded against it (durable chapters replay with zero model
// calls) and the flag is ignored, so the same resume command works with or
// without it.
...
function v4BookProductionUsage(command: "book-run" | "book-autopilot"): string {
  return `${command} <bookId> --title <title> --author <author> --v25-root <absolute> --attempt-root <absolute> --source-git-sha <sha> [--source-text <absolute>] [--resume-run-id <id>] [--research-run-id <id>] [--reconcile-unsettled] [--regen] [--max-repair 1] [--rubric-bar <60-95>] [--reader-concurrency N] [--compile-concurrency N] [--promote-local] [--no-publish]${command === "book-run" ? " [--log <absolute>]" : ""}`;
}
...
async function runV4BookProduction(
  ...
  const unsupported = firstUnsupportedFlag(flags, [
    ...
    "max-repair", "rubric-bar", "reader-concurrency", "compile-concurrency", "promote-local", "no-publish", "reconcile-unsettled", ...(command === "book-run" ? ["log"] : []),
  ]);
  if (unsupported !== undefined) {
...
Commands:
  ...
  ${v4BookProductionUsage("book-run")}
                                     ...
                                     --resume-run-id continues an existing run. The research run a round creates is
                                     recorded durably against its control run before the first chapter is researched,
                                     so a resume continues THAT research run — not whichever compatible bundle for the
                                     book is newest: the bibliography and chapter map are reused from the frozen
                                     source, every chapter whose succeeded sidecar still passes reuse validation is
                                     loaded from disk with zero model calls; a sidecar that no longer validates (e.g.
                                     it predates the current source-v2 route rules) is re-researched and logged
                                     "durable sidecar failed reuse validation — re-researching", as are the chapters
                                     with no sidecar at all (same attempt budget). Two exceptions, both loud: a run
                                     started before this binding existed (or whose record was lost) has no research
                                     run of its own, so it falls back to the newest compatible bundle and prints
                                     action=NO_RECORDED_RESEARCH_RUN; and a resume whose own research run is gone,
                                     unreadable, or no longer matches the current fingerprint (codeVersion,
                                     promptHash, configHash, provider, model — e.g. the prompts changed since the run
                                     started) FAILS CLOSED with RESEARCH_RESUME_CONFLICT naming the field, rather than
                                     silently re-researching or adopting another run's bundle; start a fresh run to
                                     research it again. Passing --regen alongside --resume-run-id changes nothing
                                     about research — on a resumed round --regen means ONLY "bypass the
                                     already-promoted pointer", so --resume-run-id X and --regen --resume-run-id X do
                                     the same work and neither is a definition conflict. Research refresh is a
                                     FIRST-round axis: --regen on a fresh run still discards any cached bundle and
                                     researches into a new research run. A resume still fails closed when the intent
                                     that matters changed (different book, title, author, --v25-root, --source-text
                                     bytes or --source-git-sha).
