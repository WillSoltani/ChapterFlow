...
Commands:
  ...
  ${v4BookProductionUsage("book-run")}
                                     ...
                                     recorded durably against its control run before the first chapter is researched, so
                                     a resume continues THAT research run — not whichever compatible bundle for the book
                                     is newest: the bibliography and chapter map are reused from the frozen source, every
                                     chapter with a succeeded sidecar in that run is loaded from disk with zero model
                                     calls, and only the chapters without one are re-run (same attempt budget). Two
                                     exceptions, both loud: a run started before this binding existed (or whose record
                                     was lost) has no research run of its own, so it falls back to the newest compatible
                                     bundle and prints action=NO_RECORDED_RESEARCH_RUN; and a resume whose own research
                                     run is gone, unreadable, or no longer matches the current fingerprint (codeVersion,
                                     ...
                                     research it again. Passing --regen alongside --resume-run-id changes nothing about
                                     research — on a resumed round --regen means ONLY "bypass the already-promoted
                                     pointer", so --resume-run-id X and --regen --resume-run-id X do the same work and
                                     neither is a definition conflict. Research refresh is a FIRST-round axis: --regen on
                                     a fresh run still discards any cached bundle and researches into a new research run.
                                     A resume still fails closed when the intent that matters changed (different book,
                                     title, author, --v25-root, --source-text bytes or --source-git-sha).
