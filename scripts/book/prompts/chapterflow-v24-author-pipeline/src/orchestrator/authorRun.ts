...
export const AUTHOR_QUALITY_BAR =
  ...
  "7. EXAMPLE CRAFT [SCORED]. Each example is a lived decision, not a slot-filler: a named actor (or a research-supported number — never an invented fact), a specific stake, and a visible before→after the move produces. Reject any scenario with no name, no number, AND no movement, that only restates the lesson, or that would fit any chapter — that reads as manufactured filler.";
