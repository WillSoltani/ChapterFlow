...
import {
  ...
  PILOT_ROLE_READINESS_V6_DIR_REL_PATH as PILOT_ROLE_READINESS_DIR_REL_PATH,
  PILOT_ROLE_READINESS_V6_EXPERIMENT_ID as PILOT_ROLE_READINESS_EXPERIMENT_ID,
  materializePilotRoleReadinessV6 as materializePilotRoleReadiness,
...
function readinessImplementationPaths(): string[] {
  return [
    ...
    `${PILOT_ROLE_READINESS_DIR_REL_PATH}/readiness-corpus.v6.json`,
    `${PILOT_ROLE_READINESS_DIR_REL_PATH}/readiness-plan.v6.json`,
...
export async function runPilotRoleReadinessCampaign(
  ...
  requireCondition(materialized.planWritten && materialized.planSha256 !== null,
    "readiness plan is not minted — mint it with pilot-role-readiness-v6 --write --mint-plan AFTER the final candidate re-mint");
