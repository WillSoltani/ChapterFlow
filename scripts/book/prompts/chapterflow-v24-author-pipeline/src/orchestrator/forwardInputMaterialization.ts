...
export const IMP22_FORWARD_INPUT_EXPECTED_HASHES = Object.freeze({
  freezeSha256: "be0cbdc1874d853ab7698b2de42721910c3abdaeb67a985c5401d4b271972f0c",
  pilot: Object.freeze({
    "radical-candor": "e7b3de09bfea7578443c7ac48df5e5c4183910ecee038837cc6d7e59c115c856",
    "start-with-why": "c33ade4e9daed3eb44e35c6b98597dd9f9ff13a39fb44b562185256d28390f28",
  }),
...
function committedBookInputRoot(experimentId: string, bookId: string): string {
  return resolve(DEFAULT_STATE_ROOT, experimentId, "inputs", bookId);
}
...
const PILOT_LOCATIONS: readonly SourceInputLocation[] = Object.freeze([
  {
    ...
    packetDir: preferExistingDirectory(
      resolve(PIPELINE_DIR, "state/books/radical-candor/runs/v23-current/source-packets"),
      resolve(committedBookInputRoot(PILOT_EXPERIMENT_ID, "radical-candor"), "books/radical-candor/runs/imp22-inputs-v1/source-packets"),
    ),
    sidecarDir: preferExistingDirectory(
      resolve(PIPELINE_DIR, ".chapterflow/runs/radical-candor/20260709T191414229Z-5258177d-971e-472b-b1ca-476d903c8a03/sidecars/source"),
      resolve(committedBookInputRoot(PILOT_EXPERIMENT_ID, "radical-candor"), "source-archive/radical-candor"),
    ),
  },
  {
    ...
    packetDir: preferExistingDirectory(
      resolve(PIPELINE_DIR, "state/books/start-with-why/runs/v23-current/source-packets"),
      resolve(committedBookInputRoot(PILOT_EXPERIMENT_ID, "start-with-why"), "books/start-with-why/runs/imp22-inputs-v1/source-packets"),
    ),
    sidecarDir: preferExistingDirectory(
      resolve(PIPELINE_DIR, ".chapterflow/runs/start-with-why/20260704T125509997Z-823c4c45-75d0-4e18-8d33-ba66548d6e21/sidecars/source"),
      resolve(committedBookInputRoot(PILOT_EXPERIMENT_ID, "start-with-why"), "source-archive/start-with-why"),
    ),
  },
]);
...
const GOLD_LOCATION: SourceInputLocation = Object.freeze({
  ...
  sidecarDir: preferExistingDirectory(
    resolve(REPO_ROOT, ".chapterflow/runs/the-gifts-of-imperfection/20260614-194740/sidecars/source"),
    resolve(committedBookInputRoot(GOLD_EXPERIMENT_ID, "the-gifts-of-imperfection"), "source-archive/the-gifts-of-imperfection"),
  ),
});
...
export function materializeImp22ForwardInputs(
  ...
  requireCondition(
    freeze.goldInputHash === IMP22_FORWARD_INPUT_EXPECTED_HASHES.gold,
    `IMP-22 gold input hash drift (${freeze.goldInputHash} != ${IMP22_FORWARD_INPUT_EXPECTED_HASHES.gold})`,
  );
  for (const [bookId, expected] of Object.entries(IMP22_FORWARD_INPUT_EXPECTED_HASHES.pilot)) {
    requireCondition(
      freeze.pilotInputHashes[bookId] === expected,
      `IMP-22 pilot input hash drift for ${bookId} (${freeze.pilotInputHashes[bookId]} != ${expected})`,
    );
  }
