...
const SECTION_REPAIR_RULES: readonly string[] = [
    ...
    "- Anchor rule: when an example or summary cites a namedExample anchor, include at least two of that anchor's hardSpecifics verbatim (SEC33/SEC13/SEC14); quiz and action units need only one (SEC56/SEC74) — do not staple extra identifier sentences to hit a quota; whyItMatters must explain the same sourceFactIds and the decision shown.",
    "- The example pack's characters are fictional and exist only there — never name them in tryThisNow or the implementation plan (SEC119, action-pack blocker): the reader has no \"Sophie\" to hand work to. Translate the mechanism into a behavior. (Quiz/summary may narrate the chapter's real cases; do not invent a recurring example protagonist there either.)",
    "- Never staple a standalone case-identifier sentence into a quiz stem, choice, or explanation (\"…while a Houston public charter network stages a college acceptance celebration.\"); if a case is named, the question must hinge on it.",
    "- Never instruct the reader to write or recite a source label in an action (\"write '60-second painful trial beside 90-second trial with milder ending'\"); translate the mechanism into a behavior.",
