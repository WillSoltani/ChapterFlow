...
function finalizeQcRoundUnlocked(bookId: string, roundId: string, options: { chapters?: number[]; attest?: boolean; dryRun?: boolean } = {}): FinalizeQcRoundResult {
  ...
  const keyResolution = resolveManualKeyJudges(bookId, roundId, chapters);
  ...
  for (const ch of chapters) {
    ...
    } else if (actionableRevisionBlock) {
      // Actionable content/gate findings must take precedence over missing author/session
      // provenance. A chapter with a quote-backed confirm REVISE (or bar/gate failure)
      // needs a writer repair, and that repair/assembly will stamp fresh provenance.
      // If provenance is checked first, the round falls into NEEDS_MORE_QC with open
      // repair evidence and autopilot misroutes it as an infra/missing-reviewer halt.
      finalVerdict = "REVISE";
