...
function parseNumberWords(raw: string): number | null {
  ...
  for (const t of tokens) {
    if (t in NUMBER_WORDS) { current += NUMBER_WORDS[t]; saw = true; continue; }
    if (t === "hundred") { current = (current || 1) * 100; saw = true; continue; }
    if (t in NUMBER_SCALES) { total += (current || 1) * NUMBER_SCALES[t]; current = 0; saw = true; continue; }
    return null;
  }
...
function digitTokens(text: string): string[] {
  ...
  const out = (src.match(NUMERIC_TOKEN_RE) ?? []).map((t) => t.replace(/,/g, ""));
  // Source-verify notes and v2 hardSpecifics often write small quantities as words
  // ("four acute care hospitals", "over four years"). The factual_accuracy QC
  // rubric trusts the groundedNumbers list; v22.0 only extracted digit numerals, so
  // verified word-quantities were invisible to QC and caused first-round false
  // REVISEs. Add word-derived tokens from VERIFIED items only.
  for (const m of src.matchAll(NUMBER_WORD_RUN_RE)) {
    const parsed = parseNumberWords(m[0]);
    if (parsed !== null) out.push(String(parsed));
  }
  return out;
}
