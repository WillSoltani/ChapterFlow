...
import { collectQcRound, submitQcArtifact } from "../src/qc/orchestrator/index.js";
import { loadBarReadArtifact, orchestratorRoundDir, submissionsDir } from "../src/qc/orchestrator/artifacts.js";
...
function barSubmissionPath(path: string): string {
  ...
  writeFileSync(path, JSON.stringify({
    ...
    roundId: ROUND,
    role: "bar",
    reviewer: "codex-qc:r-submit-token:bar:ch01",
    chapterNumber: 1,
    chapterId: `${BOOK}-ch01`,
    contentHash: "hash-ch01",
    sourceHash: "source-hash",
    axes: BAR_AXES.map((axis) => ({ axis, score: 0.9, tier: "PUBLISHABLE", hits: [] })),
    notes: "Green bar read.",
  }, null, 2), "utf8");
  return path;
}
...
test("collect prefers verified qc-submit bar evidence over loose working JSON in submissions dir", () => {
  try {
    cleanup();
    const { tokens } = openQcRound(BOOK, ROUND);
    const barDir = submissionsDir(BOOK, ROUND, "bar");
    mkdirSync(barDir, { recursive: true });
    barSubmissionPath(resolve(barDir, "ch01-working-draft.json"));

    const submitted = withSession("verified-bar-session", () =>
      submitQcArtifact(BOOK, ROUND, "bar", barSubmissionPath(resolve(TMP_DIR, "bar-submit.json")), tokens.bar),
    );
    ...
    const artifact = loadBarReadArtifact(BOOK, ROUND, 1);
    assert.equal(artifact?.reviewerSessionId, "verified-bar-session");
  } finally {
    cleanup();
  }
});
