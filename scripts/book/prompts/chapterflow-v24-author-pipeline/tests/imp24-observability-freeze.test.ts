...
const IMP24D_OBSERVABILITY_IMPLEMENTATION_COMMIT = "649235cc138ad88795b86a62e39e36d0068f8b7f";
const R1_STATE_REL =
...
test("IMP-24D advances only the execution identity while IMP-24C remains pinned to r1", () => {
  ...
  assert.equal(IMP24_ROLE_QUALIFICATION_R2_EXECUTION_ID,
...
test("IMP-24D observability freeze is deterministic, binds r1/current evidence, and creates no r2 root", () => {
  ...
  try {
    ...
    git(repositoryRoot, ["checkout", "--detach", IMP24D_OBSERVABILITY_IMPLEMENTATION_COMMIT]);
    const r2Root = retained(repositoryRoot,
    ...
    assert.equal(first.freeze.successor.executionId, IMP24_ROLE_QUALIFICATION_R2_EXECUTION_ID);
    ...
    for (const path of Object.values(IMP24D_OBSERVABILITY_FREEZE_PATHS)) {
      assert.equal(existsSync(retained(repositoryRoot, path)), true, path);
    }

    const before = walkRootManifest(repositoryRoot);
    ...
    assert.deepEqual(walkRootManifest(repositoryRoot), before, "verify mode must write nothing");

    writeFileAtomic(retained(repositoryRoot, IMP24D_OBSERVABILITY_FREEZE_PATHS.markdown), "drift\n");
    assert.throws(() => verifyImp24DObservabilityFreeze(options), /differs byte-for-byte/);

    writeFileAtomic(retained(repositoryRoot, IMP24D_OBSERVABILITY_FREEZE_PATHS.markdown),
      materialized.outputs.markdown.bytes);
    writeFileAtomic(retained(repositoryRoot, `${R1_STATE_REL}/candidate-availability.json`), "{}\n");
...
test("later correction and final checkouts verify Commit-A observability-freeze bytes historically", () => {
  ...
  try {
    ...
    const historicalFreeze = JSON.parse(readFileSync(
      retained(repositoryRoot, IMP24D_OBSERVABILITY_FREEZE_PATHS.json), "utf8",
    )) as ReturnType<typeof buildImp24DObservabilityFreeze>["freeze"];
    const observabilityCommit = IMP24D_OBSERVABILITY_IMPLEMENTATION_COMMIT;
    const original = historicalFreeze.currentImplementation;
    ...
    assert.equal(verified.freezeSha256, historicalFreeze.freezeSha256);
    assert.equal(verified.frozenSemanticsSha256,
      hashCanonical(historicalFreeze.frozenSemantics));
