...
**Options:** (a) keep auto-publish default (consistency with book-run; D7-gated; reversible) — current; (b) flip to halt-at-ready default, --publish opt-in (RECOMMENDED — the operator explicitly authorizes each push; matches the cautious posture the whole program has taken).
**Recommendation:** (b). **Default action:** none until a live run is authorized (D-3); the flip is a one-line default change bookable into WP-802 (pilot) — no code churn now. Cannot fire in any current path (no live run authorized).
